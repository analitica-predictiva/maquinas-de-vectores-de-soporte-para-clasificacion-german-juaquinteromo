...
def pregunta_03():
    ...
    columnTransformer = make_column_transformer(
        (
            OneHotEncoder(),  # Se aplica OneHotEncoder
            make_column_selector(dtype_include=object)  # Selecciona columnas tipo texto
        ),
        remainder='passthrough',  # Las columnas no especificadas se dejan pasar sin cambios
    )

    # Cree un pipeline que contenga el columnTransformer y el modelo SVC.
    pipeline = Pipeline(
        steps=[
            ("column_transformer", columnTransformer),  # Paso para transformar las columnas
            ("svc", SVC()),  # Paso para el modelo SVC
        ],
    )
